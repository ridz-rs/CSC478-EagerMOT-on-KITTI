...
from dataset_classes.kitti.mot_kitti import MOTSequenceKITTI, MOTFrameKITTI
...
from configs.params import NUSCENES_BEST_PARAMS, KITTI_BEST_PARAMS
from configs.local_variables import SPLIT, NUSCENES_WORK_DIR, MOUNT_PATH, KITTI_WORK_DIR
...
def visualize_3d(dataset, params, mode: str, output_folder_prefix: str,
    ...
    for sequence_name in target_sequences:
        ...
        img_folder_path = os.path.join(
            f"{output_folder_prefix}_{mode}_{SPLIT}_rad{int(radius)}_{'CP' if ('/centerpoint_val/') in result_json else 'ours'}_{current_time_str}".replace(":", "_"), sequence_name)
        if save_img:
            io.makedirs_if_new(img_folder_path)
            if result_json:
                with open(f"{img_folder_path}\\render.txt", "w") as file:
        ...
        for frame_i, frame_name in enumerate(all_frames[target_frame:]):
            ...
            if mode == "mot":
                ...
                assert isinstance(sequence, MOTSequenceNuScenes) or isinstance(sequence, MOTSequenceKITTI)
...
def vis_mot_predictions_submitted(frame: MOTFrame, params: Mapping, all_colors, tracking_results, radius,
                                 world: bool = False) -> List[o3d.geometry.Geometry3D]:
    assert world
    assert isinstance(frame, MOTFrameNuScenes) or isinstance(frame, MOTFrameKITTI)
